...
For technology stack choices marked as inherited (TECH-08 through TECH-12), the most recent stable release available at the start of development was selected. Versions in parentheses are recorded for traceability — the binding constraint is the technology itself, not a specific version. Upgrades to newer releases are routine architecture work, not a constraint change.
...
| TECH-04 | OpenAI APIs for AI workloads | OpenAI Whisper for transcription, OpenAI Chat Completion for content generation. The integration point per neighbour must be replaceable without touching the rest of the application. Mandated by [`CON-3c-01`](../spec/P1-constraints.md#con-3c-01-openai-apis-speech-to-text-and-chat-completion); the portability requirement is [`NFR-14c-01`](../spec/N1-nichtfunktional.md#14c-adaptability-requirements). |
...
| TECH-08 | PHP + Laravel | Server-side runtime and framework (PHP 8.5, Laravel 13.4 at start of development). Binding for all further server-side architecture: routing, persistence, validation, queuing primitives, AI SDK via `laravel/ai`. |
| TECH-09 | Vue + TypeScript + Inertia.js + Vuetify | Browser UI stack (Vue 3.5, TypeScript 6, Inertia.js 3, Vuetify 4 at start of development). Inertia bridges Laravel routing/auth and Vue rendering — no separate JSON API for the browser. Decided in [ADR-001](A09-architecture-decisions.md). |
...
| TECH-11 | SQLite as application database | Concrete realisation of TECH-03 (SQLite 3.51 at start of development). Persisted via Docker volume in development; flat file on the host in production. Never overwritten by deployment. |
| TECH-12 | Vite + Node LTS at build time only | Frontend bundling toolchain (Vite 8 with the Rolldown bundler, Node 24 LTS at start of development). Node is not part of the production runtime — only built assets are deployed. |
