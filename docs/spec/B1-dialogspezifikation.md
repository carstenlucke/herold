...
| **Validation** | Dialogue-level: API key field must not be empty. Algorithmic verification of the key against the stored credential lives in [N2.4](N2-querschnittskonzepte.md#n24-authentication-and-session). |
...
| **Qualities** | [NFR-13a-01](N1-nichtfunktional.md) *Mobile and Desktop Usage* (the recording region is the dominant target on small screens — see *Mobile usage* in [§B1.4.9](#b149-mobile-usage)); [NFR-15a-03](N1-nichtfunktional.md) *Audio Upload Validation*. |
...
Per [NFR-13a-01](N1-nichtfunktional.md) *Mobile and Desktop Usage*, every screen is usable on a current mobile browser (Safari iOS, Chrome Android) as well as a current desktop browser. The application chrome ([§B1.4.1](#b141-application-chrome)) collapses from a side rail to a bottom-edge bar on small viewports; primary actions (notably *Record* on [DLG-09](#dlg-09--capture-voice-note) and *Save* on [DLG-08](#dlg-08--note-detail)'s editing variant) remain reachable with a single thumb in portrait orientation. [DLG-09](#dlg-09--capture-voice-note) specifically is laid out so that the record/stop control is the dominant target while capture is in progress, since this is the use case most often invoked in motion.
...
| [N2](N2-querschnittskonzepte.md) | [N2.3](N2-querschnittskonzepte.md#n23-validation) underpins B1 form-validation feedback; [N2.4](N2-querschnittskonzepte.md#n24-authentication-and-session) underpins [DLG-01](#dlg-01--sign-in-first-factor) to [DLG-05](#dlg-05--recovery-result) and the unauthenticated-redirect pattern in B1.4; [N2.5](N2-querschnittskonzepte.md#n25-failure-handling) is the source for the *Synchronous error handling* pattern; [N2.7](N2-querschnittskonzepte.md#n27-content-sanitisation) is the policy behind the sanitisation step that B1 cites for the editing variant of [DLG-08](#dlg-08--note-detail). |
