...
- *Redaction is a hard rule.* The operator's API key, the operator's TOTP secret, third-party API tokens (the OpenAI API key, the GitHub Personal Access Token), and any session or bearer token must be masked before the entry leaves the application. This is the binding requirement [NFR-15b-03](N1-nichtfunktional.md) *Secret Redaction in Logs* and is enforced uniformly, regardless of the call site. Framework-level secrets (e.g. session signing keys) inherit the same rule via the chosen runtime's redaction conventions; their concrete handling is an architecture concern.
