...
| ID | Function | Purpose |
|----|----------|---------|
| AF-01 | Transcription | Convert a recorded audio document into a plain-text transcript. |
| AF-02 | Content Generation | Derive structured note content (title, body, optional extra fields) from a transcript under a given message type. |
| AF-03 | Markdown Sanitisation | Reduce arbitrary markdown to a safe subset suitable for persistence and downstream dispatch. |
| AF-04 | Message Type Resolution | Resolve a message-type identifier to its prompt, optional extra-field schema, and outbound label. |
| AF-05 | Issue Composition & Dispatch | Assemble a GitHub Issue from a note record and push it to the configured repository, recording the reference. |
| AF-06 | Status Transition | Enforce the legal state machine for a voice note (`recorded → processed → sent`). |
| AF-07 | Audio Lifecycle | Manage the audio document attached to a voice note from creation through removal. |
| AF-08 | Per-Type Input Validation | Validate operator-supplied input against the schema declared by the selected message type. |
...
- **Rules and invariants.**
  - The function is synchronous and blocks the surrounding request (per ADR-002 and [NFR-12a-01](N1-nichtfunktional.md) *Synchronous Processing*).
  - The transcript is preserved verbatim; no editorial transformation occurs in this function.
  - Failure of the upstream provider is propagated to the surrounding UC; the note's status does not advance, in line with [NFR-12d-01](N1-nichtfunktional.md) *Synchronous Error Handling*.
...
- **Rules and invariants.**
  - The prompt used is selected exclusively via AF-04; this function does not embed type knowledge.
  - The operator may overwrite any output field in the subsequent review step; this function does not lock results.
  - Markdown produced here is **not** considered sanitised — AF-03 must run before persistence and before dispatch.
- **Used by.** UC-06 *Process voice note*.
...
- **Rules and invariants.**
  - Removes constructs that can carry executable or rendering side-effects (script content, raw HTML beyond an allowlist, javascript URLs, etc.), as required by [NFR-15b-04](N1-nichtfunktional.md) *Issue Content Sanitization*.
  - Idempotent: applying the function to its own output yields the same output.
  - Preserves operator intent for the legitimate subset (headings, lists, links, code blocks, emphasis).
...
- **Rules and invariants.**
  - The set of valid identifiers is configurable; an unknown identifier is rejected.
  - All consumers of message-type behaviour go through this function — message types are never hard-coded elsewhere in the application functions.
...
- **Rules and invariants.**
  - One-way only. The function does not read back issue state after creation; no synchronisation logic lives here (per P1 non-goal [NG-03](P1-ziele-rahmenbedingungen.md) *Local ticket lifecycle*).
  - Synchronous within the dispatch request ([NFR-12a-01](N1-nichtfunktional.md)). On success, the note transitions to `sent` via AF-06; on failure, the status is not advanced and the operator can retry per [NFR-12d-01](N1-nichtfunktional.md).
  - The pushed body must already be sanitised (AF-03) per [NFR-15b-04](N1-nichtfunktional.md).
  - Exactly one type label is attached; further triage is the consumer's responsibility (F1.3).
...
- **Rules and invariants.**
  - Allowed transitions: `recorded → processed`, `processed → sent`. No other transitions, including no regressions.
  - Each transition is the consequence of a successful operation in another function (AF-02 for `processed`, AF-05 for `sent`); status is never advanced speculatively.
...
- **Rules and invariants.**
  - Audio is persisted alongside the note at recording time (UC-05) and bound to it.
  - Audio is streamable for the entire lifetime of the note (UC-10); there is no scheduled pruning, no retention timer, and no `processed`-triggered cleanup. The applicable size and format constraints at upload are governed by [NFR-15a-03](N1-nichtfunktional.md) *Audio Upload Validation*.
  - Audio is removed together with the note record when the operator deletes the note (UC-11). Removal is irreversible.
  - The note record may exist without an audio document only after deletion of the audio (which currently coincides with note deletion); audio never exists without a note.
...
- **Rules and invariants.**
  - The schema comes from AF-04; this function does not embed per-type knowledge.
  - Validation is strict: missing required fields, wrong value ranges, or unknown fields all fail.
  - The audio recording itself is not validated here beyond presence; format checks live closer to AF-01.
