...
- **`<<extend>>` from [UC-06](#uc-06--process-voice-note), [UC-07](#uc-07--edit-generated-content), [UC-08](#uc-08--dispatch-voice-note) and [UC-11](#uc-11--delete-a-voice-note) to [UC-10](#uc-10--view-a-voice-note):** at the `detail view action` extension point in [UC-10](#uc-10--view-a-voice-note), the operator may optionally trigger one of these note-flow use cases. Each extension is guarded by the note status it requires — `status = recorded` for [UC-06](#uc-06--process-voice-note), `status = processed` for [UC-07](#uc-07--edit-generated-content) and [UC-08](#uc-08--dispatch-voice-note); [UC-11](#uc-11--delete-a-voice-note) is reachable in any status and carries no guard. The guards are written on the diagram edges because UML *extend* legitimately carries an extension condition; the same guards also appear in each use case's *Precondition* row for self-contained reading.
...
[UC-10](#uc-10--view-a-voice-note) thus acts as the UI hub from which the operator reaches the note-flow actions, modelled as four extensions at a single `detail view action` extension point. The choice of `<<extend>>` over `<<include>>` records that [UC-10](#uc-10--view-a-voice-note) is complete in itself (postcondition: *no state change*) and that triggering any of the four note-flow use cases is an optional, condition-guarded operator decision — not a mandatory sub-flow of [UC-10](#uc-10--view-a-voice-note).
...
| **Actors** | Operator (primary). |
| **Precondition** | Operator has out-of-band write access to the host (e.g. via FTP) and has placed a recovery token there within the last 60 minutes (see [NFR-15a-04](N1-nichtfunktional.md) *Recovery Token Expiry*). The token's content is a secret string of the operator's choice. |
| **Postcondition** | Authenticated session active. The bound TOTP is unbound (`Operator.totpSecret` and `Operator.totpConfirmedAt` cleared); a fresh API key has been generated, persisted as `Operator.apiKeyHash`, and shown to the operator exactly once. The recovery-token file has been deleted. |
| **Main scenario** | 1. Operator selects the recovery option from the sign-in screen.<br>2. System checks that a recovery token exists and has not expired (see [NFR-15a-04](N1-nichtfunktional.md) *Recovery Token Expiry*).<br>3. Operator enters the token's secret string.<br>4. System verifies the entered string in constant time against the file's content, then deletes the recovery-token file.<br>5. System unbinds the TOTP secret, generates a fresh API key, persists its hash, and establishes an authenticated session.<br>6. System displays the new API key to the operator exactly once; the operator records it.<br><br>![UC-03 Recover access — main scenario](diagrams-png/f2-uc03-recover-access.png) |
| **Alternative scenarios** | - *Recovery completes:* continues with [UC-02](#uc-02--enrol-second-factor) *Enrol second factor* to re-establish the second factor before normal use resumes. |
...
| **Main scenario** | 1. Operator opens the recording screen.<br>2. Operator selects a message type.<br>3. If the type declares extra fields (per [D2.4](D2-datentypen.md#d24-messagetypedt) *MessageTypeDT* and [D2.7](D2-datentypen.md#d27-typespecificdata) *TypeSpecificData*), system reveals them and operator fills the required ones.<br>4. Operator records an audio note in the browser.<br>5. Operator submits the recording.<br>6. System validates the operator input against the type schema ([D2.7](D2-datentypen.md#d27-typespecificdata)) and the audio against the upload constraints in [NFR-15a-03](N1-nichtfunktional.md) *Audio Upload Validation*.<br>7. System persists the audio document (lifecycle invariants per [D1.1](D1-datenmodell.md#voicenote)) and a new note record, transitioning it to status `recorded` ([D2.5](D2-datentypen.md#d25-notestatusdt)).<br><br>![UC-05 Capture voice note — main scenario](diagrams-png/f2-uc05-capture-note.png) |
...
| **Qualities** | [NFR-15a-03](N1-nichtfunktional.md) *Audio Upload Validation*; [NFR-13a-01](N1-nichtfunktional.md) *Mobile and Desktop Usage*. |
...
| **Main scenario** | 1. Operator opens the note's detail view (see [UC-10](#uc-10--view-a-voice-note)).<br>2. Operator triggers processing.<br>3. System transcribes the audio via [S1.3](S1-nachbarsysteme.md#s13--nb-02--openai-whisper-api) *Whisper API*.<br>4. System resolves the message type ([D2.4](D2-datentypen.md#d24-messagetypedt)) and generates structured content from the transcript via [S1.4](S1-nachbarsysteme.md#s14--nb-03--openai-chat-completion-api) *Chat Completion API*.<br>5. System sanitises the generated markdown ([AF-03](F3-anwendungsfunktionen.md#af-03--markdown-sanitisation)) and persists the structured content.<br>6. System transitions the note to status `processed` ([D2.5](D2-datentypen.md#d25-notestatusdt)).<br><br>![UC-06 Process voice note — main scenario](diagrams-png/f2-uc06-process-note.png) |
...
| **Main scenario** | 1. Operator opens the note's detail view (see [UC-10](#uc-10--view-a-voice-note)).<br>2. Operator edits the title, body, or extra fields.<br>3. Operator saves.<br>4. System sanitises the edited markdown ([AF-03](F3-anwendungsfunktionen.md#af-03--markdown-sanitisation)) per [NFR-15b-04](N1-nichtfunktional.md) *Issue Content Sanitization*, revalidates the extra fields against the type schema ([D2.7](D2-datentypen.md#d27-typespecificdata)), and persists the changes.<br><br>![UC-07 Edit generated content — main scenario](diagrams-png/f2-uc07-edit-content.png) |
...
| **Main scenario** | 1. Operator opens the note's detail view (see [UC-10](#uc-10--view-a-voice-note)).<br>2. Operator triggers dispatch.<br>3. System composes a GitHub issue from the note using the type-resolved label ([D2.4](D2-datentypen.md#d24-messagetypedt)) and the sanitised content ([AF-03](F3-anwendungsfunktionen.md#af-03--markdown-sanitisation)).<br>4. System pushes the issue to the configured GitHub repository via [S1.5](S1-nachbarsysteme.md#s15--nb-04--github-issues-api) *Issues API*.<br>5. System records the resulting issue reference against the note and transitions it to status `sent` ([D2.5](D2-datentypen.md#d25-notestatusdt)).<br><br>![UC-08 Dispatch voice note — main scenario](diagrams-png/f2-uc08-dispatch-note.png) |
...
| **Postcondition** | No state change. |
| **Main scenario** | 1. System renders the note's detail view: status, type, timestamps, structured content (if any), issue reference (if `sent`).<br>2. Operator may stream the audio recording (lifecycle invariants per [D1.1](D1-datenmodell.md#voicenote)). |
...
- **Transcription, content generation, GitHub dispatch.** Calls into neighbouring systems with no operator decision point — see [S1.3](S1-nachbarsysteme.md#s13--nb-02--openai-whisper-api), [S1.4](S1-nachbarsysteme.md#s14--nb-03--openai-chat-completion-api), [S1.5](S1-nachbarsysteme.md#s15--nb-04--github-issues-api).
- **Markdown sanitisation.** Local algorithm with no operator decision point — see [F3.AF-03](F3-anwendungsfunktionen.md#af-03--markdown-sanitisation).
...
| [F3](F3-anwendungsfunktionen.md) | [AF-03](F3-anwendungsfunktionen.md#af-03--markdown-sanitisation) *Markdown Sanitisation* runs inside [UC-06](#uc-06--process-voice-note), [UC-07](#uc-07--edit-generated-content), [UC-08](#uc-08--dispatch-voice-note). |
| [S1](S1-nachbarsysteme.md) | [S1.3](S1-nachbarsysteme.md#s13--nb-02--openai-whisper-api), [S1.4](S1-nachbarsysteme.md#s14--nb-03--openai-chat-completion-api) are invoked by [UC-06](#uc-06--process-voice-note); [S1.5](S1-nachbarsysteme.md#s15--nb-04--github-issues-api) by [UC-08](#uc-08--dispatch-voice-note). |
| [D1](D1-datenmodell.md) | Voice note record, audio document lifecycle invariants, issue reference, message-type metadata are referenced throughout. [UC-11](#uc-11--delete-a-voice-note) is the only deleter. |
| [D2](D2-datentypen.md) | Status machine for `VoiceNote.status` ([D2.5](D2-datentypen.md#d25-notestatusdt)) governs transitions in [UC-05](#uc-05--capture-voice-note), [UC-06](#uc-06--process-voice-note), [UC-08](#uc-08--dispatch-voice-note); message-type bindings ([D2.4](D2-datentypen.md#d24-messagetypedt)) drive [UC-05](#uc-05--capture-voice-note), [UC-06](#uc-06--process-voice-note), [UC-08](#uc-08--dispatch-voice-note); the slot inventory ([D2.7](D2-datentypen.md#d27-typespecificdata)) is validated at [UC-05](#uc-05--capture-voice-note) and [UC-07](#uc-07--edit-generated-content). |
...
| [N2](N2-querschnittskonzepte.md) | [N2.4](N2-querschnittskonzepte.md#n24-authentication-and-session) underpins [UC-01](#uc-01--sign-in) to [UC-04](#uc-04--sign-out) and gates every other use case; [N2.2](N2-querschnittskonzepte.md#n22-type-driven-configuration) and [N2.3](N2-querschnittskonzepte.md#n23-validation) cut across [UC-05](#uc-05--capture-voice-note) and [UC-07](#uc-07--edit-generated-content); [N2.5](N2-querschnittskonzepte.md#n25-failure-handling) governs failures of [UC-06](#uc-06--process-voice-note) and [UC-08](#uc-08--dispatch-voice-note); [N2.7](N2-querschnittskonzepte.md#n27-content-sanitisation) runs in [UC-06](#uc-06--process-voice-note), [UC-07](#uc-07--edit-generated-content), [UC-08](#uc-08--dispatch-voice-note); [N2.6](N2-querschnittskonzepte.md#n26-logging) and [N2.8](N2-querschnittskonzepte.md#n28-secret-handling) apply throughout. |
