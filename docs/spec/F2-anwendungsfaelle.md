...
| **Main scenario** | 1. Operator opens the recording screen.<br>2. Operator selects a message type.<br>3. If the type declares extra fields (per [AF-04](F3-anwendungsfunktionen.md#af-04--message-type-resolution)), system reveals them and operator fills the required ones.<br>4. Operator records an audio note in the browser.<br>5. Operator submits the recording.<br>6. System validates the operator input against the type schema ([AF-08](F3-anwendungsfunktionen.md#af-08--per-type-input-validation)) and the audio against the upload constraints in [NFR-15a-03](N1-nichtfunktional.md) *Audio Upload Validation*.<br>7. System persists the audio document ([AF-07](F3-anwendungsfunktionen.md#af-07--audio-lifecycle)) and a new note record, transitioning it to status `recorded` ([AF-06](F3-anwendungsfunktionen.md#af-06--status-transition)).<br><br>![UC-05 Capture voice note — main scenario](diagrams-png/f2-uc05-capture-note.png) |
...
| **Main scenario** | 1. Operator opens the note's detail view (see [UC-10](#uc-10--view-a-voice-note)).<br>2. Operator triggers processing.<br>3. System transcribes the audio ([AF-01](F3-anwendungsfunktionen.md#af-01--transcription)).<br>4. System resolves the message type ([AF-04](F3-anwendungsfunktionen.md#af-04--message-type-resolution)) and generates structured content from the transcript ([AF-02](F3-anwendungsfunktionen.md#af-02--content-generation)).<br>5. System sanitises the generated markdown ([AF-03](F3-anwendungsfunktionen.md#af-03--markdown-sanitisation)) and persists the structured content.<br>6. System transitions the note to status `processed` ([AF-06](F3-anwendungsfunktionen.md#af-06--status-transition)).<br><br>![UC-06 Process voice note — main scenario](diagrams-png/f2-uc06-process-note.png) |
...
| **Main scenario** | 1. Operator opens the note's detail view (see [UC-10](#uc-10--view-a-voice-note)).<br>2. Operator triggers dispatch.<br>3. System composes a GitHub issue from the note ([AF-05](F3-anwendungsfunktionen.md#af-05--issue-composition--dispatch)) using the type-resolved label ([AF-04](F3-anwendungsfunktionen.md#af-04--message-type-resolution)) and the sanitised content ([AF-03](F3-anwendungsfunktionen.md#af-03--markdown-sanitisation)).<br>4. System pushes the issue to the configured GitHub repository.<br>5. System records the resulting issue reference against the note and transitions it to status `sent` ([AF-06](F3-anwendungsfunktionen.md#af-06--status-transition)).<br><br>![UC-08 Dispatch voice note — main scenario](diagrams-png/f2-uc08-dispatch-note.png) |
...
| **Postcondition** | No state change. |
| **Main scenario** | 1. System renders the note's detail view: status, type, timestamps, structured content (if any), issue reference (if `sent`).<br>2. Operator may stream the audio recording ([AF-07](F3-anwendungsfunktionen.md#af-07--audio-lifecycle)). |
...
- **Re-process and re-dispatch on failure.** Operator simply repeats [UC-06](#uc-06--process-voice-note) or [UC-08](#uc-08--dispatch-voice-note); the status machine ([AF-06](F3-anwendungsfunktionen.md#af-06--status-transition)) makes retries safe.
