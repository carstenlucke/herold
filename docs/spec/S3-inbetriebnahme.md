...
|--------|-------------|--------|
| **Application runtime** | The language and framework runtime required by the architecture is available on the host. The runtime is invoked per HTTP request; no long-running application process is expected. | [`docs/arch/`](../arch/) |
| **Persistent storage** | A writable area on the host file store that is *not* overwritten by deployments. It holds the application database ([CON-3a-03](P1-constraints.md#con-3a-03-low-footprint-database)), the captured audio documents referenced by `VoiceNote.audioPath` ([D1.1](D1-datenmodell.md#voicenote)), the application log ([N2.6](N2-querschnittskonzepte.md#n26-logging)), and the recovery-token drop point used by [UC-03](F2-anwendungsfaelle.md#uc-03--recover-access). | [CON-3a-03](P1-constraints.md#con-3a-03-low-footprint-database), [NFR-14a-02](N1-nichtfunktional.md) |
...
|---------|-------|----------|-------|
| **Application database** | Operator | From first commissioning onwards. | The low-footprint database surface mandated by [CON-3a-03](P1-constraints.md#con-3a-03-low-footprint-database). Holds the entities of [D1](D1-datenmodell.md). Initialised empty at first commissioning; never reset by a deployment. |
...
| **Host configuration** | IT / host operator | Provisioned at first commissioning; updated out-of-band thereafter. | Carries the third-party credentials catalogued in [N2.8](N2-querschnittskonzepte.md#n28-secret-handling) (the OpenAI API key and the GitHub Personal Access Token), the per-`MessageTypeDT` host-config bindings ([NFR-14a-01](N1-nichtfunktional.md)), and the AI provider selection ([NFR-14c-01](N1-nichtfunktional.md)), alongside whatever framework-level setup the chosen runtime requires (an architecture concern, not specified here). Never observable from the browser ([NFR-15b-01](N1-nichtfunktional.md), [NFR-15b-02](N1-nichtfunktional.md)). |
...
2. **Provision host configuration.** The IT / host operator places the host-side configuration described in [S3.3](#s33-persistent-state-surfaces) — third-party credentials, per-`MessageTypeDT` bindings ([NFR-14a-01](N1-nichtfunktional.md)), AI provider selection ([NFR-14c-01](N1-nichtfunktional.md)), plus the framework-level setup the chosen runtime requires. Configuration is opaque to the application until it is read at request time; misconfiguration surfaces as a runtime failure on the affected use case.
...
4. **Initialise the empty database.** The application database surface ([CON-3a-03](P1-constraints.md#con-3a-03-low-footprint-database)) is created and the schema is established through the optional shell channel of [S3.2](#s32-host-preconditions). After this step the application's persistent surfaces of [S3.3](#s33-persistent-state-surfaces) exist and are empty (no `VoiceNote`, no `Operator`, no `RecoveryToken`).
...
1. **Off-host build.** The release artefact is produced off the host so that the host does not need build tooling (an architecture concern; the host runtime declared in [S3.2](#s32-host-preconditions) is the only execution surface required). The artefact is a deterministic function of the released source revision.
...
4. **Post-release verification.** A short pass through [F1](F1-geschaeftsprozesse.md) confirms that the pipeline still completes end-to-end. Operator action is not otherwise required: an active session continues to satisfy the gate of [N2.4](N2-querschnittskonzepte.md#n24-authentication-and-session) across the upload, provided the host configuration remains stable across releases.
...
|-------|-----------------|
| [P1](P1-ziele-rahmenbedingungen.md) | [CON-3a-03](P1-constraints.md#con-3a-03-low-footprint-database), [CON-3a-04](P1-constraints.md#con-3a-04-single-user-system), [CON-3b-01](P1-constraints.md#con-3b-01-shared-hosting-production) bound the host preconditions of [S3.2](#s32-host-preconditions); [NG-04](P1-ziele-rahmenbedingungen.md) excludes scheduler-driven commissioning steps. |
