...
Voice-based task dispatcher for local AI agents. Laravel 13 monolith with Inertia.js + Vue 3 + Vuetify 4. See `prompts/herold-spec.prompt.md` for full specification, `DESIGN.md` for UI design guidelines, and `docs/arch/A09-architecture-decisions.md` for ADRs.
...
- **Diagrams:** PlantUML sources live in `docs/spec/diagrams/` (spec-level, implementation-free) and `docs/arch/diagrams/` (architecture-level, implementation-bound); generated PNGs land in the sibling `diagrams-png/` next to each source directory. When changing the database schema (migrations, models, enums), update `docs/arch/diagrams/relational-datamodel.plantuml`. Regenerate all PNGs with `./scripts/generate-diagrams.sh`.
