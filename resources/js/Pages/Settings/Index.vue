        ...
        <v-card-text class="pa-4">
          <div class="d-flex align-center justify-space-between">
            <div class="d-flex align-center ga-3">
              <v-icon :icon="isDark ? 'mdi-weather-night' : 'mdi-weather-sunny'" size="22" color="warning" />
              <div>
                <div class="text-body-1 font-weight-medium" style="color: var(--text-primary)">Theme</div>
                <div class="text-caption" style="color: var(--text-muted)">
                  {{ isDark ? 'Dark mode (Neon Tokyo)' : 'Light mode' }}
                </div>
              </div>
            </div>
            <v-switch
...
function toggleTheme() {
  theme.global.name.value = isDark.value ? 'heroldDark' : 'heroldLight'
}
